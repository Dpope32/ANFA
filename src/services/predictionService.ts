import { StockData, PredictionResult, PredictionScenario, AccuracyMetrics, ModelStats } from "../types";
...
export class PredictionService {
   ...
   */
  async predict(stockData: StockData, timeframe: string = "30d"): Promise<PredictionResult> {
    try {
      ...
      const basePrediction = await this.polynomialRegression.predict(model, timeframe);
  ...
  private extractFeatures(stockData: StockData): any {
    ...
    const politicalSignal = this.calculatePoliticalSignal(stockData.politicalTrades);
    const insiderSignal = this.calculateInsiderSignal(stockData.insiderActivity);
    const optionsSignal = this.calculateOptionsSignal(stockData.optionsFlow);
   ...
   */
  private async calculateAccuracy(model: any, features: any): Promise<AccuracyMetrics> {
    ...
    // Calculate MAPE
    const mape = actualPrices.reduce((sum, actual, i) => {
      const predicted = predictions[i] || actual;
      return sum + Math.abs((actual - predicted) / actual);
    }, 0) / actualPrices.length * 100;
   ...
   */
  private calculateConfidence(accuracy: AccuracyMetrics, stockData: StockData): number {
    ...
    if (stockData.politicalTrades && stockData.politicalTrades.length > 0) confidence += 0.1;
    if (stockData.insiderActivity && stockData.insiderActivity.length > 0) confidence += 0.1;
    if (stockData.optionsFlow && stockData.optionsFlow.length > 0) confidence += 0.1;
