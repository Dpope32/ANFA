import { StockData, PredictionResult, ChartData, PoliticalEvent, VolumeAnomaly } from "../types";
...
export class ChartService {
   ...
   */
  async generateChartData(stockData: StockData, prediction: PredictionResult): Promise<ChartData> {
    try {
      ...
      const predictions = this.generatePredictionLines(historical, prediction, dates);
      
      // Generate political events
      const politicalEvents = this.generatePoliticalEvents(stockData.politicalTrades);
      
      // Generate volume anomalies
      const volumeAnomalies = this.generateVolumeAnomalies(stockData.marketData.volume);
   ...
   */
  private generatePredictionDates(historical: any[], prediction: PredictionResult): Date[] {
    const dates: Date[] = [];
   ...
   */
  private generatePredictionLines(historical: any[], prediction: PredictionResult, dates: Date[]): any {
   ...
   */
  private generateScenarioLine(startPrice: number, targetPrice: number, probability: number, length: number): number[] {
    const line: number[] = [];
    ...
    for (let i = 0; i < length; i++) {
      ...
      const noise = (Math.random() - 0.5) * (1 - probability) * Math.abs(priceChange) * 0.1;
  ...
  private generateVolumeAnomalies(volumeData: any[]): VolumeAnomaly[] {
    ...
    const volumeStd = Math.sqrt(volumes.reduce((sum, vol) => sum + Math.pow(vol - averageVolume, 2), 0) / volumes.length);
    ...
    volumeData.forEach((point, index) => {
      ...
      if (zScore > 2) {
        ...
        anomalies.push({
          ...
          averageVolume,
          anomalyScore,
          description: `Volume spike: ${(volume / averageVolume * 100).toFixed(0)}% of average`,
        });
      }
    });
