...
export class PoliticalTradingAnalyzer {
  ...
  analyzePoliticalSentiment(
    ...
    for (const trade of politicalTrades) {
      ...
      partyBreakdown[trade.party] = (partyBreakdown[trade.party] || 0) + tradeAnalysis.weight;
      chamberBreakdown[trade.chamber] = (chamberBreakdown[trade.chamber] || 0) + tradeAnalysis.weight;
  ...
  analyzeInsiderSentiment(
    ...
    for (const activity of insiderActivity) {
      const activityAnalysis = this.analyzeIndividualInsiderActivity(activity, symbol);
      ...
      titleBreakdown[activity.title] = (titleBreakdown[activity.title] || 0) + activityAnalysis.weight;
    ...
    const sentiment = totalWeight > 0 ? totalSentiment / totalWeight : 0;
    const confidence = this.calculateInsiderConfidence(insiderActivity, totalWeight);
    ...
    return {
      ...
      volumeAnalysis: {
        totalVolume,
        averageTradeSize: insiderActivity.length > 0 ? totalVolume / insiderActivity.length : 0,
        largestTrade
      }
    };
  }
  ...
  detectUnusualPoliticalActivity(politicalTrades: PoliticianTrade[]): {
    ...
    const highValueTrades = recentTrades.filter(trade => trade.amount >= this.impactThresholds.political.high);
    if (highValueTrades.length > 0) {
      reasons.push(`${highValueTrades.length} recent high-value political trades (>$1M)`);
      riskScore += 0.4;
    }
    ...
    for (const [party, trades] of Object.entries(partyGroups)) {
      if (trades.length >= 3) {
        ...
        if (recentPartyTrades.length >= 3) {
          reasons.push(`Coordinated ${party} trading activity (${recentPartyTrades.length} trades)`);
          riskScore += 0.3;
        }
      }
    }
    ...
    const senateTrades = politicalTrades.filter(trade => trade.chamber === "Senate");
    const houseTrades = politicalTrades.filter(trade => trade.chamber === "House");
  ...
  detectUnusualInsiderActivity(insiderActivity: InsiderActivity[]): {
    ...
    const highValueActivity = recentActivity.filter(activity => 
      activity.value >= this.impactThresholds.insider.high
    );
    
    if (highValueActivity.length > 0) {
      reasons.push(`${highValueActivity.length} recent high-value insider trades (>$5M)`);
      riskScore += 0.5;
    }
    ...
    const sellTrades = insiderActivity.filter(activity => activity.tradeType === "SELL");
    const buyTrades = insiderActivity.filter(activity => activity.tradeType === "BUY");
  ...
  applyPoliticalAndInsiderAdjustments(
    ...
    if (politicalAnalysis.confidence > 0.3) {
      const politicalAdjustment = politicalAnalysis.sentiment * politicalAnalysis.impactScore * 0.05;
      ...
      if (Math.abs(politicalAdjustment) > 0.01) {
        ...
        factors.push(`${direction} political trading signals (${(politicalAdjustment * 100).toFixed(1)}%)`);
      }
    ...
    if (insiderAnalysis.confidence > 0.3) {
      const insiderAdjustment = insiderAnalysis.sentiment * insiderAnalysis.impactScore * 0.03;
      ...
      if (Math.abs(insiderAdjustment) > 0.01) {
        ...
        factors.push(`${direction} insider activity (${(insiderAdjustment * 100).toFixed(1)}%)`);
      }
    ...
    if (politicalAnalysis.recentActivity && politicalAnalysis.impactScore > 0.7) {
   ...
   */
  private analyzeIndividualTrade(trade: PoliticianTrade, symbol: string): {
    sentiment: number;
    weight: number;
    impactScore: number;
    isRecent: boolean;
    ...
    const partyInfluenceMultiplier = (this.partyInfluence as any)[trade.party]?.[sector] || 1.0;
    ...
    // Calculate weight
    const weight = timingWeight * partyInfluenceMultiplier * chamberMultiplier * impactScore;

    // Generate factor description
    const factor = `${trade.politician} (${trade.party} ${trade.chamber}) ${trade.tradeType.toLowerCase()} $${(trade.amount / 1000).toFixed(0)}K`;
   ...
   */
  private analyzeIndividualInsiderActivity(activity: InsiderActivity, symbol: string): {
    sentiment: number;
    weight: number;
    impactScore: number;
    isRecent: boolean;
    ...
    // Generate factor description
    const factor = `${activity.insider} (${activity.title}) ${activity.tradeType.toLowerCase()} ${activity.shares.toLocaleString()} shares`;
   ...
   */
  private calculateConfidence(trades: PoliticianTrade[], totalWeight: number): number {
   ...
   */
  private calculateInsiderConfidence(activities: InsiderActivity[], totalWeight: number): number {
  ...
  private getSectorFromSymbol(symbol: string): string {
    const techSymbols = ["AAPL", "MSFT", "GOOGL", "AMZN", "NVDA", "AMD", "TSLA"];
   ...
   */
  private groupTradesByParty(trades: PoliticianTrade[]): { [party: string]: PoliticianTrade[] } {
