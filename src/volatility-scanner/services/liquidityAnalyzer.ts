...
export class LiquidityAnalyzer {
  ...
  async analyzeLiquidity(symbol: string): Promise<LiquidityMetrics> {
    ...
    const atmSpread = atmCall && atmCall.bid && atmCall.ask ? 
      (atmCall.ask - atmCall.bid) / ((atmCall.ask + atmCall.bid) / 2) : avgSpread;
  ...
  async isLiquid(
    ...
    return metrics.optionsVolume >= minVolume && 
   ...
   */
  async getMostLiquidStrikes(symbol: string, count: number = 5): Promise<any[]> {
    const optionsChain = await this.polygonOptions.getOptionsChain(symbol);
  ...
  async getLiquidityScore(symbol: string): Promise<number> {
    ...
    else if (metrics.bidAskSpread.average < 0.10) score += 10;
