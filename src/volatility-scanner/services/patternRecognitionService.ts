...
export class PatternRecognitionService {
   ...
   */
  private async detectVolatilitySqueeze(
    symbol: string
  ): Promise<Pattern | null> {
    const volatility =
      await this.volatilityService.calculateEmpiricalVolatility(symbol);
    const priceResponse = await this.polygonClient.getCurrentPrice(symbol);

    if (!priceResponse?.data) return null;

    // Check for squeeze conditions
    const isSqueezing =
      volatility.hv20 < volatility.hv60 * 0.7 && // Volatility contracting
      volatility.percentile < 30; // Low volatility percentile

    if (!isSqueezing) return null;

    const currentPrice = priceResponse.data.price || 100;
  ...
  private async detectReversal(symbol: string): Promise<Pattern | null> {
    ...
    if (!priceResponse?.data || priceResponse.data.length < 5) return null;

    const priceData = priceResponse.data;
    const currentPrice = priceData[priceData.length - 1].close;
    const high5Day = Math.max(...priceData.slice(-5).map((p: any) => p.high));
    const low5Day = Math.min(...priceData.slice(-5).map((p: any) => p.low));

    // Check for reversal conditions
    const isBullishReversal =
      currentPrice < low5Day * 1.02 && // Near recent lows
      priceData[priceData.length - 1].transactions >
        priceData[priceData.length - 2].transactions * 1.5; // Volume spike

    const isBearishReversal =
      currentPrice > high5Day * 0.98 && // Near recent highs
      priceData[priceData.length - 1].transactions >
        priceData[priceData.length - 2].transactions * 1.5;
  ...
  private async detectBreakout(symbol: string): Promise<Pattern | null> {
    ...
    if (!priceResponse?.data || priceResponse.data.length < 20) return null;

    const priceData = priceResponse.data;
    const currentPrice = priceData[priceData.length - 1].close;
    const high20Day = Math.max(...priceData.map((p: any) => p.high));
    const low20Day = Math.min(...priceData.map((p: any) => p.low));
    const avgVolume =
      priceData.reduce((sum: number, p: any) => sum + p.transactions, 0) /
      priceData.length;
    const currentVolume = priceData[priceData.length - 1].transactions;

    // Check for breakout conditions
    const isBullishBreakout =
      currentPrice > high20Day * 0.98 && // Near 20-day high
      currentVolume > avgVolume * 1.5; // Volume confirmation

    const isBearishBreakout =
      currentPrice < low20Day * 1.02 && // Near 20-day low
